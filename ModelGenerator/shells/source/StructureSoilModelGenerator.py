from utility.UtilityBase import FPMFeed, NodeItem, ElementBaseItem, FixedItem, SpdpKItem, SpdpCItem, NodeSetItem, LoadItem, CoupleItem
...
class StructSoilModelGenerator(ModelGeneratorBase):
    ...
    height = [10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10]
...
## Soil
    ...
    def _LogElementCount(self):
        self.elem_count_accumulated = len(self._elements)
        ...
        for kdx in range(0, self._soil_module_z+1):
            for jdx in range(0, self._soil_module_y+1):
                for idx in range(0, self._soil_module_x+1):
                    ...
                    if jdx == self._soil_module_y or jdx == 0:
                        if idx != self._soil_module_x and idx != 0:
                            self.nodeset_y_con.AddNode(node_item)
                            self.nodeset_disp.AddNode(node_item)
                            self._nodes.append(node_item)
                            count += 1
                    if (idx == self._soil_module_x or idx == 0) and (jdx == self._soil_module_y or jdx == 0):
    ...
    def _GenerateSoilElement(self):
        ...
        for idx in range(0, self.nodeset_x_con.Size()):
            ...
            count += 1
        for idx in range(0, self.nodeset_y_con.Size()):
            node_1 = self.nodeset_y_con.GetNode(idx)
            node_2 = self.nodeset_y.GetNode(idx)
            self._elements.append(LineElement("COMBIN", count, node_1._id, node_2._id, 2, 0, 0))
            count += 1
        for idx in range(0, self.nodeset_xy_con.Size()):
            node_1 = self.nodeset_xy_con.GetNode(idx)
            node_2 = self.nodeset_xy.GetNode(idx)
            self._elements.append(LineElement("COMBIN", count, node_1._id, node_2._id, 3, 0, 0))
            count += 1
    ...
    def _GenerateSoilSupport(self):
        ...
        # self._constraints.append(SpdpKItem("bc_x_spdp", self.nodeset_x, [[1, self.kn], [2, self.kt], [3, self.kt]]))
        # self._constraints.append(SpdpCItem("bc_x_spdp", self.nodeset_x, [[1, self.cn], [2, self.ct], [3, self.ct]]))
        # self._constraints.append(SpdpKItem("bc_y_spdp", self.nodeset_y, [[1, self.kt], [2, self.kn], [3, self.kt]]))
        # self._constraints.append(SpdpCItem("bc_y_spdp", self.nodeset_y, [[1, self.ct], [2, self.cn], [3, self.ct]]))
        pass
...
## Struct
    ...
    def _InitStructure(self, zsize, module_z):
        ModelGeneratorBase.__init__(self)
        self._struct_size_x = self._soil_size_x / self._soil_module_x * 4
        self._struct_size_y = self._soil_size_y / self._soil_module_y * 4
...
## Control
    ...
    def _WriteBegin(self, txtfile):
        ...
        txtfile.Write("1, Mat-Soil, DP, 30000, 0, 0\r\n")
    ...
    def _WriteEnd(self, txtfile):
        ...
        txtfile.Write("step_gravity, CONSTRAINT, bc_x, N, bc_y, N, bc_xy, N, bc_z, Y, bc_wall_fixall, N\r\n")
    ...
    def _WriteElements(self, txtfile):
        ...
        for element in self._elements:
            if element._type == "BEAM" and element._real != self.real:
                continue
            element.LogToTxt(txtfile)
        txtfile.Write("##\r\n")

    def _SetWriteReal(self, inreal):
        self.real = inreal
